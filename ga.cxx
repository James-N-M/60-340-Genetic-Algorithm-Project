...
namespace cs340 
{
  namespace 
  {
    ...
    struct schedule_compare 
    {
      ...
      bool operator()(schedule const& first, schedule const& second) const
      {
	return first.score(matrix_) > second.score(matrix_);
      }
      ...
      size_t const pool_size, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
      uniform_int_distribution<size_t> rd(0, matrix.machines()-1);
      ...
      std::generate_n(std::back_inserter(schedules), pool_size,
      	[&rd, &gen, &matrix]() -> schedule{
            schedule s1{matrix.tasks()};
      		for (size_t i = 0; i < matrix.tasks(); ++i) {
		        s1.set_task_assignment(i, rd(gen));
	        }
	        return s1;
      	}
      );
    ...
    auto cross_over(schedule c1, schedule const& c2, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
      uniform_int_distribution<size_t> ud(0, c1.tasks() -1);
      auto randomi = ud(gen);
      ...
      /* TODO: WRITE CODE HERE */
      for (auto i = randomi; i < c1.tasks(); ++i) {
        ...
        random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
      uniform_int_distribution<size_t> rdT(0, matrix.tasks() -1);
      auto randomTask = rdT(gen);
      uniform_int_distribution<size_t> rdM(0, matrix.machines() -1);
      auto randomMachine = rdM(gen);
      c.set_task_assignment(randomMachine, randomTask);
//      c.set_task_assignment(randomTask, randomMachine);
      ...
      vector<schedule>& gene_pool, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
        if (x_pairs_count > 0 && x_pairs_count < gene_pool.size()) {
	        ...
	        /* TODO: WRITE CODE HERE */
	        auto iter = gene_pool.begin() + gene_pool.size() - x_pairs_count;
	        gene_pool.erase(iter, gene_pool.end());
	        ...
	        std::transform(gene_pool.begin(),
	                       ...
	                       [&matrix](schedule const& s) {
		                       return s.score(matrix);
	                       });
	        ...
	        /* TODO: WRITE CODE HERE */
	        uniform_real_distribution<double> rd(0, totals.back());
	        ...
	        for (size_t i = 0; i < x_pairs_count; ++i) {
		        ...
		        /* TODO: WRITE CODE HERE */
		        auto lowerBound1 = std::lower_bound(totals.begin(), totals.end(), rand1);
		        ...
		        /* TODO: WRITE CODE HERE */
		        auto lowerBoundOffset1 = std::distance(totals.begin(), lowerBound1);

		        // 2e iv. Perform the two above steps again. This time using the
		        // second random number you drew.

		        /* TODO: WRITE CODE HERE */
		        auto lowerBound2 = std::lower_bound(totals.begin(), totals.end(), rand2);
		        auto lowerBoundOffset2 = std::distance(totals.begin(), lowerBound2);
		        ...
		        /* TODO: WRITE CODE HERE */
		        auto geneIter1 = gene_pool.begin() + lowerBoundOffset1;
		        auto geneIter2 = gene_pool.begin() + lowerBoundOffset2;

		        // 2e vi. Create a new schedule object by calling cross_over with
		        // the two schedules pointed to by your computed iterators.

		        /* TODO: WRITE CODE HERE */
		        schedule s = cross_over(*geneIter1, *geneIter2, gen);
		        ...
		        /* TODO: WRITE CODE HERE */
		        auto crossIter = std::lower_bound(gene_pool.begin(),
		                         gene_pool.end(),
		                         s,
		                         schedule_compare{matrix} );
                gene_pool.insert(crossIter, s);
      ...
      /* TODO: WRITE CODE HERE */
		uniform_int_distribution<size_t> rd(mut_dist);
		auto num_mutations = rd(gen);
      ...
      for (size_t j{}; j < num_mutations; ++j) 
      {
        ...
        /* TODO: WRITE CODE HERE */
	if (pos > solution) {
		std::rotate(solution, next(solution), pos);	
	} else {
		std::rotate(pos, solution, next(solution));
	}
      }
    }
      ...
      random_generator& gen)
  {
    ...
    for (size_t i{}; i < args.threads; ++i) 
    {
      ...
      /* TODO: WRITE CODE HERE */
	    std::vector<size_t> seeds{ud(gen),ud(gen),ud(gen),ud(gen),ud(gen),ud(gen)};
      ...
      future_winners.push_back(
        async(
          ...
          [&matrix, &args, seeds = move(seeds), i]() -> schedule
          {
            ...
            /* TODO: WRITE CODE HERE */
			auto gen = random_generator(seedSeq);
            ...
            /* TODO: WRITE CODE HERE */
	          auto gene_pool = populate_gene_pool(matrix, pool_size, gen);
    ...
    /* TODO: WRITE CODE HERE */
	for (auto& sched : future_winners) {
		auto&& winner = sched.get();
		if (schedule_compare{matrix}(winner, best)) {
